...
class TestTsharkMethods(unittest.TestCase):
    def test_tshark_get_arguments(self):
        ...
        args = ["-r", filename, "-T", "fields", "-e", "frame.time", "-e", "frame.cap_len", "-e", "eth.dst", "-e",
                "eth.src", "-e", "ip.dst", "-e", "ip.src", "-e", "ip.proto", "-e", "tcp.srcport", "-e", "tcp.dstport",
                "-e", "tcp.flags", "-e", "tcp.len", "-e", "tcp.stream", "-e", "udp.srcport", "-e", "udp.dstport", "-e",
                "udp.length", "-e", "http.request.method", "-e", "http.request.uri", "-e", "tls.handshake.version",
